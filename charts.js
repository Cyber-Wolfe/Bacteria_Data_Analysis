...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var toptenIds = result.otu_ids.slice(0,10).map(otuids => `OT ${otuids}`).reverse();
    var toptenLabels = result.otu_labels.slice(0,10).reverse();
    var toptenSampleVal = result.sample_values.slice(0,10).reverse();
  ...
  var gaugeData = {
      ...
      gauge: { 
        ...
        steps: [
          ...
          { range: [6, 8], color: "#1ED529" },
          { range: [8, 10], color: "#0BA214" },
        ]
      }
    };
